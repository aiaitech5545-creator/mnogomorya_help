...
if not BOT_TOKEN or ":" not in BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN пустой/неверный.")
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL пустой — подключи PostgreSQL.")
if not BASE_URL:
    print("WARN: BASE_URL пуст — webhook не установится.")
...
def get_sheet():
    ...
    if _sheet is None:
        ...
        ws = sh.sheet1
        headers = ["timestamp","tg_id","tg_username","name","phone","ship_type","position","experience","topic","slot_start_local","slot_end_local","payment_method","gcal_event_id"]
        try:
...
def get_calendar():
    ...
    if _gcal is None:
        if not GCAL_SA_JSON:
            print("WARN: Calendar не настроен.")
...
def create_calendar_event_sync(start_utc, end_utc, summary, description):
    try:
        ...
        ev = {"summary": summary,"description": description,
...
async def get_free_slots(session: AsyncSession) -> List[dict]:
    q = text("""SELECT id, start_utc, end_utc FROM slots WHERE is_booked=false AND start_utc>NOW() ORDER BY start_utc ASC LIMIT 12""")
...
async def reserve_slot(session: AsyncSession, slot_id: int, user_id: int) -> Optional[int]:
    ...
    res = await session.execute(q1, {"id": slot_id}); row = res.first()
    if not row: return None
    q2 = text("INSERT INTO bookings(user_id, slot_id, status, paid) VALUES (:uid,:sid,'requested',false) RETURNING id")
    return (await session.execute(q2, {"uid": user_id, "sid": slot_id})).scalar_one()
...
async def ensure_user(session: AsyncSession, tg_id: int, username: Optional[str]) -> int:
    ...
    return (await session.execute(ins, {"tg": tg_id, "un": username})).scalar_one()
...
async def start_form(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.answer("Как вас зовут? (только имя)"); await cq.answer()
...
async def cb_book(cq: CallbackQuery, state: FSMContext):
    await show_slots(cq.message); await cq.answer()

async def show_slots(target: Message):
    async with Session() as s:
        slots = await get_free_slots(s)
    if not slots:
        await target.answer("Свободных слотов пока нет. Напишите желаемое время — постараюсь подстроиться."); return
    rows, row = [], []
...
async def choose_slot(cq: CallbackQuery, state: FSMContext):
    ...
    if slot_row:
        ...
        await state.update_data(slot_start_local=human_dt(start_utc), slot_end_local=human_dt(end_utc),
                                slot_start_utc=start_utc, slot_end_utc=end_utc)
    ...
    await cq.message.answer("Выберите способ оплаты (для учёта в заявке):", reply_markup=kb); await cq.answer()
...
async def payment_pick(cq: CallbackQuery, state: FSMContext):
    ...
    gcal_event_id = ""
    try:
        start_utc = data.get("slot_start_utc"); end_utc = data.get("slot_end_utc")
        if start_utc and end_utc:
            ...
            gcal_event_id = await loop.run_in_executor(None, lambda: create_calendar_event_sync(start_utc, end_utc, summary, description))
    except Exception as e:
        print("WARN: Calendar creation failed:", e)

    try:
        ws = get_sheet(); now = datetime.utcnow().isoformat()
        ws.append_row([now, str(cq.from_user.id), data.get("tg_username") or ("@" + (cq.from_user.username or "")),
                       data.get("name"), data.get("phone") or "", data.get("ship_type"), data.get("position"),
                       data.get("experience"), data.get("topic"), data.get("slot_start_local"),
                       data.get("slot_end_local"), data.get("payment_method"), gcal_event_id or ""])
    except Exception as e:
        print("WARN: Sheets append failed:", e)

    await state.clear()
    await cq.message.answer("Спасибо! Заявка сохранена. Я свяжусь с вами для подтверждения. 🙌"); await cq.answer()

@dp.message(Command("admin"))
async def admin_menu(m: Message):
    if m.from_user.id not in ADMIN_IDS: return
    await m.answer("Админ: /addslot YYYY-MM-DD HH:MM — добавить слот (длительность берётся из SLOT_MINUTES).\nПример: /addslot 2025-10-25 15:00")
...
async def addslot(m: Message):
    ...
    try:
        ...
        local_tz = _tz.gettz(TZ); dt_local = dt_local.replace(tzinfo=local_tz)
        dt_utc = dt_local.astimezone(_tz.UTC); dt_utc_end = dt_utc + timedelta(minutes=SLOT_MINUTES)
    except Exception:
        await m.answer("Неверный формат. Пример: /addslot 2025-10-25 15:00"); return
    async with Session() as s:
        await s.execute(text("INSERT INTO slots(start_utc, end_utc, is_booked) VALUES (:s,:e,false)"), {"s": dt_utc, "e": dt_utc_end})
        await s.commit()
