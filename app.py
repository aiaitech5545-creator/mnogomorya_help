...
# --- Normalize Railway/Heroku style DATABASE_URL for SQLAlchemy+asyncpg ---
...
# Enforce SSL if not specified (Railway usually requires it)
if _raw_db_url and "sslmode=" not in _raw_db_url:
    _raw_db_url += ("?sslmode=require" if "?" not in _raw_db_url else "&sslmode=require")
...
print("Aiogram:", getattr(sys.modules.get('aiogram'), '__version__', 'unknown'))
print("BOT_TOKEN:", mask_token(BOT_TOKEN))
print("BASE_URL:", BASE_URL or "EMPTY")
print("DATABASE_URL set:", bool(DATABASE_URL))
try:
    from urllib.parse import urlparse
    _u = urlparse(DATABASE_URL) if DATABASE_URL else None
    print("DB scheme:", (_u.scheme if _u else "EMPTY"))
    print("DB host:", (_u.hostname if _u else "EMPTY"))
    print("DB sslmode?","sslmode=" in (DATABASE_URL or ""))
except Exception as _e:
    print("DIAG urlparse failed:", _e)
...
if not BOT_TOKEN or ":" not in BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN отсутствует или выглядит неверно (нет двоеточия). Проверь Variables на Railway.")

if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL отсутствует. Добавь PostgreSQL в проект Railway.")

if not BASE_URL:
    print("WARN: BASE_URL пуст. Вебхук не встанет, но сервер запустится. Укажи BASE_URL в Railway → Variables.")
...
def get_sheet():
    ...
    if _sheet is None:
        if not GSPREAD_SA_JSON or not GSPREAD_SHEET_ID:
            print("WARN: Google Sheets не настроен (нет GSPREAD_SERVICE_ACCOUNT_JSON или GSPREAD_SHEET_ID). Пропускаю инициализацию.")
...
def get_calendar():
    ...
    if _gcal is None:
        if not GCAL_SA_JSON:
            print("WARN: Google Calendar не настроен (нет GCAL_SERVICE_ACCOUNT_JSON). Пропускаю инициализацию.")
...
async def on_startup():
    if BASE_URL:
        ...
        print("INFO: BASE_URL is empty, webhook not set. Use getUpdates or set BASE_URL.")
